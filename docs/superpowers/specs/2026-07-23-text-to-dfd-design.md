...
`--background` sets the canvas colour alone. A label chip is painted
with the theme's own `ChipFill`, kept separate from `Canvas` for exactly
this reason, so repainting the page leaves the chips where the theme
put them.
